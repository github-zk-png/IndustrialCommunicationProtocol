	...
	internal class Program
	{
		...
		private static async Task OPCUA()
		{
			try
			{
				...
				await protocol.SetMonitorNode("ns=2;s=数据类型示例.16 位设备.K 寄存器.Short1", (MonitoredItem monitoredItem, MonitoredItemNotificationEventArgs e) =>
				{
				   MonitoredItemNotification? monitoredItemNotification = (MonitoredItemNotification)e.NotificationValue;
				   
				   Console.WriteLine($"节点名称: {monitoredItem.StartNodeId.ToString()}");
				   Console.WriteLine($"值: {monitoredItemNotification.Value.Value}");
				   Console.WriteLine($"类型: {monitoredItemNotification.Value.Value?.GetType().Name}");
				});

				await protocol.SetMonitorNode("ns=2;s=数据类型示例.16 位设备.K 寄存器.Short2", (MonitoredItem monitoredItem, MonitoredItemNotificationEventArgs e) =>
				{
					MonitoredItemNotification? monitoredItemNotification = (MonitoredItemNotification)e.NotificationValue;
				...
				//            var fawef = new List<OpcUaMonitorNodeParameter>
				//            {
